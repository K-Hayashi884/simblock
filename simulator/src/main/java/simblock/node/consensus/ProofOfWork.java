...
public class ProofOfWork extends AbstractConsensusAlgo {
  ...
  public MiningTask minting() {
    ...
    MiningTask task = selfNode.getSelfishNess()
        ? new SelfishMiningTask(selfNode, (long) (Math.log(u) / Math.log(1.0 - p) / selfNode.getMiningPower()),
            difficulty)
        : new MiningTask(selfNode, (long) (Math.log(u) / Math.log(1.0 - p) / selfNode.getMiningPower()), difficulty);
    return p <= Math.pow(2, -53) ? null : task;
  }
